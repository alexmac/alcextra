...
{
    char *ptr;
	__asm__ volatile("trace('PHYSFS_init')");
	...
	__asm__ volatile("trace('calculating base dir')");
	__asm volatile("trace(\"argv0 = \" + CModule.readString(%0, %1));\n"
				   : : "r"(argv0), "r"(strlen(argv0)));
    baseDir = calculateBaseDir(argv0);
	__asm volatile("trace(\"baseDir = \" + CModule.readString(%0, %1));\n"
				   : : "r"(baseDir), "r"(strlen(baseDir)));
	...
	__asm__ volatile("trace('calculating user dir')");
    userDir = calculateUserDir();
	__asm volatile("trace(\"userDir = \" + CModule.readString(%0, %1));\n"
				   : : "r"(userDir), "r"(strlen(userDir)));
...
{
    fprintf(stderr, "PHYSFS_openAppend: %s\n", filename);
                                    ...
                                    PHYSFS_uint64 len)
{
    ...
    if (rc < 0)
        ...
        if(fh->bufpos != 0) {
            __asm volatile("trace(new Error().getStackTrace());");
        }
    //assert(fh->bufpos == 0);
